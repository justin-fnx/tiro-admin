    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-lg shadow-lg">
        <div className="text-center">
          <h1 className="text-3xl font-bold text-gray-900">TIRO Admin</h1>
          <p className="mt-2 text-sm text-gray-600">관리자 전용 페이지입니다</p>
        </div>

        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md text-sm">
            {error === 'AccessDenied' && '접근이 거부되었습니다. 등록된 관리자만 로그인할 수 있습니다.'}
            {error === 'OAuthSignin' && 'Google 로그인 중 오류가 발생했습니다.'}
            {error === 'OAuthCallback' && '인증 콜백 처리 중 오류가 발생했습니다.'}
            {!['AccessDenied', 'OAuthSignin', 'OAuthCallback'].includes(error) &&
              '로그인 중 오류가 발생했습니다.'}
          </div>
        )}
